...
export function Card({ children, className, hover }: CardProps) {
  return (
    <div className={cn("bg-white border border-slate-200 rounded-xl shadow-sm", hover && "hover:shadow-md hover:border-slate-300 transition-all cursor-pointer", className)}>
      {children}
    </div>
  );
}
