...
import { Search, Plus, Filter, Copy, Check, Mail, ExternalLink } from "lucide-react";
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="relative mb-6">
        ...
        <input
          className="w-full pl-10 pr-4 py-2.5 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-500 bg-white"
      ...
      {loading ? (
        ...
        <Card>
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <h3 className="font-semibold text-slate-700 mb-2">{search ? "No results found" : "No clients yet"}</h3>
            <p className="text-sm text-slate-500 mb-5">{search ? "Try a different search term" : "Add your first client to start onboarding"}</p>
                ...
                <div className="flex items-center gap-4 px-5 py-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <Link href={`/clients/${client.id}`} className="font-semibold text-slate-900 hover:text-brand-600 transition-colors truncate">
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      ...
                      <span className="text-xs text-slate-400">{progress}% onboarded</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-3 shrink-0">
                    ...
                    <div className="flex gap-1">
                      ...
                      <Link href={`/clients/${client.id}`} className="p-2 rounded-lg hover:bg-slate-100 text-slate-400 hover:text-slate-700 transition-colors">
      ...
      {inviteOpen && (
        <Modal open={!!inviteOpen} onClose={() => setInviteOpen(null)} title="Onboarding Link Ready">
          <div className="space-y-4">
            <p className="text-sm text-slate-600">Share this link with <strong>{inviteOpen.client.name}</strong> to start their onboarding:</p>
            <div className="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-lg p-3">
              <code className="flex-1 text-xs text-slate-700 truncate">{inviteOpen.url}</code>
              <button onClick={() => copyUrl(inviteOpen.url)} className="flex-shrink-0 p-1.5 hover:bg-slate-200 rounded-md transition-colors">
                {copiedUrl ? <Check size={14} className="text-emerald-600" /> : <Copy size={14} className="text-slate-500" />}
              </button>
            </div>
            <div className="flex gap-3">
              ...
              <Button className="flex-1" onClick={() => window.open(`mailto:${inviteOpen.client.email}?subject=Your Onboarding Link&body=Hi ${inviteOpen.client.name},%0A%0AClick the link below to complete your onboarding:%0A%0A${inviteOpen.url}%0A%0ABest,`)}>
                <Mail size={14} />
                Open email
              </Button>
            </div>
