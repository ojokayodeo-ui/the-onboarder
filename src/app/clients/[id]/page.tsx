...
import { ClientEmailSequence } from "@/components/dashboard/client-email-sequence";
import { ClientTabsWrapper } from "@/components/dashboard/client-tabs-wrapper";
...
import type { ParsedActionPlan } from "@/types";
    ...
    <div className="animate-fade-in">
      ...
      <div className="flex items-start gap-4 mb-6">
        ...
        <div className="flex items-center gap-2">
          <a href={`/api/export/${client.id}?format=json`}
      ...
      {/* Onboarding progress bar — always visible */}
      <div className="mb-6 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 px-6 py-4">
        <div className="flex items-center gap-4 mb-3">
          <Progress value={progress} showLabel size="lg" className="flex-1" />
          <span className="text-xs text-slate-400 dark:text-slate-500 whitespace-nowrap">
            {client.completedAt ? `Completed ${formatDate(client.completedAt)}` : client.invitedAt ? `Invited ${formatDateRelative(client.invitedAt)}` : `Added ${formatDate(client.createdAt)}`}
          </span>
        </div>
          ...
          {sections.map(({ id, label }) => {
            ...
            return (
              <div key={id} className="text-center">
                <div className={`h-1.5 rounded-full mb-1.5 ${response?.isComplete ? "bg-emerald-500" : response ? "bg-amber-400" : "bg-slate-200 dark:bg-slate-700"}`} />
      ...
      {/* Tab navigation */}
      <ClientTabsWrapper activeTab={activeTab} clientId={client.id}>
          ...
          <div className="col-span-2 space-y-5">
                              ...
                              {Object.entries(data).map(([key, val]) => {
                                ...
                                return (
                                  <div key={key} className={Array.isArray(val) || value.length > 60 ? "col-span-2" : ""}>
                                    <dt className="text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">{key.replace(/([A-Z])/g, " $1").replace(/^./, s => s.toUpperCase())}</dt>
                      ...
                      <div className="space-y-2">
                        {client.assets.map((asset) => (
                          <a key={asset.id} href={asset.url} target="_blank" rel="noreferrer"
                            className="flex items-center gap-3 p-3 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 border border-slate-100 dark:border-slate-700 transition-colors">
            ...
            {activeTab === "emails" && (
              <Card>
                <CardHeader>
                  <div>
                    <h2 className="font-semibold text-slate-900 dark:text-white">Client Journey Emails</h2>
                    <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">7-step sequence — AI writes each email personalised to this client</p>
                  </div>
                </CardHeader>
                <CardBody>
                  <ClientEmailSequence clientId={client.id} />
                </CardBody>
              </Card>
            )}
              ...
              <Card>
                <CardHeader>
                  <div>
                    ...
                    <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Direct emails to {client.name} — AI can draft for you</p>
                  </div>
                </CardHeader>
                <CardBody>
